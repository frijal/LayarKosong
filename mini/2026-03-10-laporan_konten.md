# 🔆 Laporan Ringkas Konten
Update: 10 Mar 2026 | 11:35 WITA

### 📑 Statistik Global
| Ekstensi | Jumlah | Lokasi Folder |
| :--- | :---: | :--- |
| 📄 /Dockerfile | **1** | Dockerfile |
| 📄 /LICENSE | **1** | LICENSE |
| 📄 /_headers | **1** | _headers |
| 📄 /_redirects | **1** | _redirects |
| 📄 /artikeljson | **1** | artikeljson |
| 📄 /artikelx/jurukunci | **1** | artikelx |
| 📄 /img/jurukunci | **1** | img |
| 📄 /sementara/audit/jurukunci | **1** | sementara |
| 📄 /sementara/jurukunci | **1** | sementara |
| 🎨 css   | **48** | dapur, ext |
| 📄 gif   | **1** | img |
| 🎨 html  | **2167** | sitemap.html, dapur, warta-tekno, img.html, artikel, security-policy.html, ext, privacy.html, index.html, sistem-terbuka, search.html, data-deletion.html, disclaimer.html, gaya-hidup, olah-media, jejak-sejarah, llms-index.html, feed.html, 404.html, opini-sosial, lisensi.html, about.html, lainnya, google2e9cfaf2f2923d27.html, data-deletion-form.html |
| 📄 htmlY | **1** | img.htmlY |
| 📄 ico   | **4** | dapur, artikel, ext, favicon.ico |
| 🖼️ jpg   | **2** | artikelx, thumbnail.jpg |
| 📜 js    | **93** | dapur, artikel, ext, functions, sementara |
| 📄 jsX   | **3** | dapur |
| 📄 json  | **14** | dapur, artikel, ext, manifest.json, package.json, kat-lainnya.json, kat-sistem-terbuka.json, tsconfig.json, kat-opini-sosial.json, kat-olah-media.json, kat-jejak-sejarah.json, kat-warta-tekno.json, kat-gaya-hidup.json, artikel.json |
| 📄 lock  | **1** | bun.lock |
| 📝 md    | **6** | SECURITY.md, README.md, disclaimer.md, CONTRIBUTING.md, CODE_OF_CONDUCT.md, llms.md |
| 📄 pl    | **2** | dapur, artikel |
| 🖼️ png   | **101** | img, dapur, logo.png, favicon.png, artikel, ext, thumbnail.png |
| 📜 py    | **5** | dapur, artikel, sementara |
| 📜 sh    | **16** | dapur, artikel, artikelx, sementara |
| 📄 svg   | **50** | img, dapur, artikel, ext, favicon.svg |
| 📄 toml  | **1** | wrangler.toml |
| 📄 ts    | **51** | dapur, sementara |
| 📄 tsX   | **5** | dapur |
| 📄 tsXXX | **1** | dapur |
| 📄 tsY   | **1** | dapur |
| 📄 txt   | **36** | llms.txt, img, dapur, artikel, artikelx, changed_files.txt, EdD8dkmdNLlxREi2LkhJjYOH2kyQbJqM3cBKT5fX.txt, sementara, sitemap.txt, daftar-isi.txt, ads.txt, robots.txt |
| 📄 webmanifest | **1** | site.webmanifest |
| 🖼️ webp  | **406** | img, thumbnail.webp |
| 📄 woff2 | **8** | dapur, ext |
| 📄 xml   | **14** | feed-olah-media.xml, opensearch.xml, sitemap.xml, video-sitemap-1.xml, feed-opini-sosial.xml, image-sitemap-1.xml, BingSiteAuth.xml, feed-lainnya.xml, feed-sistem-terbuka.xml, feed-gaya-hidup.xml, rss.xml, feed-jejak-sejarah.xml, feed-warta-tekno.xml |
| 📄 xsl   | **1** | sitemap-style.xsl |

**Total File:** 3048
