...
const facebookRegex =
  /<div class="fb-comments".*?<\/div>|<div id="fb-root"><\/div>|<script.*?src=".*?connect\.facebook\.net.*?<\/script>\s*/gs;
// Menampilkan pesan header sesuai mode yang aktif
if (isDryRun) {
  console.log("--- 🛡️  MODE SIMULASI (DRY RUN) AKTIF 🛡️  ---");
  console.log(
    "Tidak ada file yang akan diubah. Skrip hanya akan melaporkan.\n",
  );
} else {
  console.log("--- ⚡ MODE EKSEKUSI (LIVE) AKTIF ⚡ ---");
  console.log(
    "PERINGATAN: Perubahan pada file akan disimpan secara permanen.\n",
  );
}
console.log(
  `Memulai pemindaian file .html di folder: ${path.resolve(targetFolder)}`,
);
let filesChanged = 0;
