...
async function processFile(file: string, baseUrl: string, fallbackTime: string) {
  ...
  // 🔧 FIX: --- 1C. MIRRORING GAMBAR DALAM ELEMEN SVG <image href="..."> ---
  // Elemen <image> di dalam SVG inline (dipakai untuk diagram, infografis interaktif, dll)
  // TIDAK memakai atribut "src" seperti <img> biasa, melainkan "href" (spesifikasi SVG2)
  // atau "xlink:href" (SVG1.1 lama). Tanpa blok ini, gambar semacam ini lolos begitu saja
  // dari proses mirroring dan tetap menunjuk ke domain eksternal (mis. raw.githubusercontent.com).
  const svgImageElements = $("image").toArray();
  await Promise.all(svgImageElements.map(async (el) => {
    const $img = $(el);
    ...
    const src = $img.attr(hrefAttr);
    if (src && src.startsWith("http")) {
      const localPath = await mirrorAndConvert(src, baseUrl);
      if (localPath.startsWith("/img")) {
        $img.attr(hrefAttr, `${baseUrl}${localPath}`);
      } else {
        console.warn(`   ↳ <image> SVG tetap memakai URL eksternal (mirror gagal): ${src}`);
      }
    }
  }));
