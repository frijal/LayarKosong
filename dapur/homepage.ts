  ...
  try {
    ...
    for (const cat in data) {
      ...
      data[cat].forEach((item: any) => {
        ...
        // Strip emoji depan (kalau ada) + suffix "- Layar Kosong" — samain persis
        // dengan sanitize() + strip suffix yang dipakai komposisi.ts pas SSR,
        // biar card yang di-render build-time nggak beda tampilan sama yang
        // di-render client-side pas kamu search/filter/klik "muat lebih banyak".
        const cleanTitle = item.title.replace(/^\p{Emoji_Presentation}\s*/u, '').replace(/\s*-\s*Layar Kosong$/i, '');
