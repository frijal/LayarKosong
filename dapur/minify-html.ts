...
let stats: Stats = {
	success: 0, skipped: 0, failed: 0, errorList: [], totalSaved: 0, totalBefore: 0, totalAfter: 0
};
// ========== UTILITIES ==========
...
async function processFile(filePath: string): Promise<void> {
	try {
		...
		// TAHAP 3: INJEKSI BALIK & SIGNATURE
		...
		minifiedHTML = minifiedHTML.includes('</body>') 
		? minifiedHTML.replace('</body>', `${signature}</body>`) 
		: minifiedHTML + signature;
...
async function run(): Promise<void> {
	...
	// Hajar semua sekaligus tanpa batching/concurrency limit
