...
function savePostedUrls(urls: string[]): void {
  ...
  // Gunakan append standar agar tidak menimpa log lama
  appendFileSync(LOG_FILE, data, "utf-8");
}

function getSemesterRange(targetDate: Date) {
  const year = targetDate.getFullYear();
  const month = targetDate.getMonth(); // 0 = Jan, 11 = Des
...
async function buildSemesterAggregation() {
  ...
  while (allPendingArticles.length > 0) {
    ...
    const { start, end } = getSemesterRange(firstArt.date);

    if (today <= end) {
      const semLabel = start.getMonth() === 0 ? "Jan-Jun" : "Jul-Des";
      console.log(`⏳ Semester ${start.getFullYear()} (${semLabel}) belum berakhir.`);
      break;
    }
    ...
    const labelSemester = start.getMonth() === 0 ? "Semester 1" : "Semester 2";
    const fileName = `agregat-${start.getFullYear()}-${labelSemester.replace(/\s+/g, "").toLowerCase()}.html`;
    const pageUrl = `https://dalam.web.id/artikel/${fileName}`;
    const mainCover = currentBatch[0].thumb;
    const titlePage = `Arsip Layar Kosong: ${labelSemester} ${start.getFullYear()}`;
...
<body>
    <div class="container">
        <header>
            <h1>${labelSemester} (${start.getFullYear()})</h1>
