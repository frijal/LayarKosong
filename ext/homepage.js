var X=[],W=[],k=[],_=0,U=null,L=6,P="All";async function T(){try{let q=await fetch("artikel.json");if(!q.ok)throw Error("Gagal load JSON");let z=await q.json();X=[];for(let J in z){let K=J.toLowerCase().replace(/\s+/g,"-");z[J].forEach((Q)=>{let R=Q[1],O=R.endsWith(".html")?R.replace(/\.html$/,""):R;X.push({category:J,title:Q[0],url:`/${K}/${O}`,img:Q[2],date:new Date(Q[3]),summary:Q[4]||""})})}X.sort((J,K)=>K.date.getTime()-J.date.getTime()),W=[...X],k=[...new Set(X.map((J)=>J.category))].map((J)=>X.find((K)=>K.category===J)),v(),G()}catch(q){console.error("Gagal ambil data",q);let z=document.getElementById("newsFeed");if(z)z.innerHTML="<p>Gagal memuat konten.</p>"}}function v(){F(),b(),f(),$(),V();let q=document.getElementById("searchInput"),z=document.getElementById("clearSearch"),j=document.getElementById("hero");if(q)q.addEventListener("input",(Q)=>{let R=Q.target.value.toLowerCase();if(R.length>0){if(j)j.style.display="none";Y()}else{if(j)j.style.display="block";G()}W=X.filter((O)=>O.title.toLowerCase().includes(R)||O.summary&&O.summary.toLowerCase().includes(R)),V(!0),$()});if(z&&q)z.addEventListener("click",()=>{if(q.value="",j)j.style.display="block";W=[...X],V(!0),$(),G(),q.focus()});let J=document.getElementById("yearFilter"),K=document.getElementById("monthFilter");if(J)J.addEventListener("change",()=>{N(),M()});if(K)K.addEventListener("change",M)}function $(q=P){P=q;let z=document.getElementById("sidebarRandom");if(!z)return;z.innerHTML="";let j=P==="All"?X:X.filter((O)=>O.category===P),J=[],K=new Set,R=Math.min(7,j.length);while(J.length<R){let O=Math.floor(Math.random()*j.length);if(!K.has(O))J.push(j[O]),K.add(O)}z.innerHTML=J.map((O)=>{let Z=O.title.replace(/"/g,"&quot;"),w=O.date,B=`${String(w.getDate()).padStart(2,"0")}-${String(w.getMonth()+1).padStart(2,"0")}-${String(w.getFullYear()).slice(-2)}`;return`
    ...
    <span style="color: var(--text-muted, #777); font-size: 0.7rem; opacity: 0.8;">• ${B}</span>
    </div>
    </div>
    </div>`}).join("")}window.renderSidebar=$;function E(q,z){if(z)document.querySelectorAll(".pill").forEach((j)=>j.classList.remove("active")),z.classList.add("active");W=q==="All"?[...X]:X.filter((j)=>j.category===q),V(!0),$(q)}window.filterByCat=E;function F(){if(k.length===0)return;let q=document.getElementById("hero"),z=document.getElementById("heroSliderWrapper");if(!q||!z)return;q.classList.remove("skeleton"),z.innerHTML=k.map((K)=>`
  ...
  </div>`,J=q.querySelector(".hero-nav");if(J)J.remove();q.insertAdjacentHTML("beforeend",j),document.getElementById("heroPrev")?.addEventListener("click",(K)=>{K.preventDefault(),A(-1)}),document.getElementById("heroNext")?.addEventListener("click",(K)=>{K.preventDefault(),A(1)}),q.addEventListener("mouseenter",Y),q.addEventListener("mouseleave",G),C()}function C(){let q=document.getElementById("heroSliderWrapper");if(!q)return;let z=_*100;q.style.transform=`translateX(-${z}%)`,document.querySelectorAll(".hero-slide").forEach((J,K)=>{J.classList.toggle("active",K===_)})}function G(){if(U)clearInterval(U);U=setInterval(()=>{_=(_+1)%k.length,C()},4600)}function Y(){if(U)clearInterval(U),U=null}function A(q){if(_+=q,_>=k.length)_=0;else if(_<0)_=k.length-1;C(),Y(),G()}function V(q=!1){if(q)L=6;let z=document.getElementById("newsFeed");if(!z)return;z.innerHTML="";let j=document.getElementById("hero"),J=j&&j.style.display!=="none",K=k.map((Z)=>Z.title),Q=W.filter((Z)=>{if(J&&K.includes(Z.title))return!1;return!0});Q.slice(0,L).forEach((Z)=>{z.innerHTML+=`
