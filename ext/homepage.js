  ...
  try {
    ...
    for (const cat in data) {
      ...
      data[cat].forEach((item) => {
        let fileName = item[1];
        // Hapus .html jika ada
        let fileSlug = fileName.endsWith(".html")
    ...
    heroData = categories.map((cat) =>
      allData.find((item) => item.category === cat),
    );
...
function renderSidebar() {
  ...
  const isHeroVisible = heroSection && heroSection.style.display !== "none";
  const titlesInHero = heroData.map((h) => h.title);
  const displayedTitles = displayedData
    .slice(0, limit)
    .map((item) => item.title);
...
function renderCategories() {
  ...
  const pillsHTML = cats
    .map(
      (c) => `<div class="pill" onclick="filterByCat('${c}', this)">${c}</div>`,
    )
    .join("");

  container.innerHTML =
    `<div class="pill active" onclick="filterByCat('All', this)">Kategori</div>` +
    pillsHTML;
}

function renderArchives() {
  const years = [...new Set(allData.map((i) => i.date.getFullYear()))].sort(
    (a, b) => b - a,
  );
  ...
  years.forEach(
    (y) => (ySelect.innerHTML += `<option value="${y}">${y}</option>`),
  );
  updateMonthDropdown();
}
