var R=[],$=[],_=[],Z=0,j=null,W=6;async function E(){try{let q=await fetch("artikel.json");if(!q.ok)throw Error("Gagal load JSON");let z=await q.json();R=[];for(let G in z){let O=G.toLowerCase().replace(/\s+/g,"-");z[G].forEach((J)=>{let X=J[1],U=X.endsWith(".html")?X.replace(/\.html$/,""):X;R.push({category:G,title:J[0],url:`/${O}/${U}`,img:J[2],date:new Date(J[3]),summary:J[4]||""})})}R.sort((G,O)=>O.date.getTime()-G.date.getTime()),$=[...R],_=[...new Set(R.map((G)=>G.category))].map((G)=>R.find((O)=>O.category===G)),N(),V()}catch(q){console.error("Gagal ambil data",q);let z=document.getElementById("newsFeed");if(z)z.innerHTML="<p>Gagal memuat konten.</p>"}}function N(){v(),B(),M(),A(),k();let q=document.getElementById("searchInput"),z=document.getElementById("hero");if(q)q.addEventListener("input",(J)=>{let U=J.target.value.toLowerCase();if(U.length>0){if(z)z.style.display="none";L()}else{if(z)z.style.display="block";V()}$=R.filter((Q)=>Q.title.toLowerCase().includes(U)||Q.summary&&Q.summary.toLowerCase().includes(U)),k(!0),A()});let K=document.getElementById("clearSearch");if(K&&q)K.addEventListener("click",()=>{if(q.value="",z)z.style.display="block";$=[...R],k(!0),A(),V(),q.focus()});let G=document.getElementById("yearFilter"),O=document.getElementById("monthFilter");if(G)G.onchange=Y;if(O)O.onchange=Y}function v(){if(_.length===0)return;let q=document.getElementById("hero"),z=document.getElementById("heroSliderWrapper");if(!q||!z)return;q.classList.remove("skeleton"),z.innerHTML=_.map((O)=>`
  ...
  `,G=q.querySelector(".hero-nav");if(G)G.remove();q.insertAdjacentHTML("beforeend",K),document.getElementById("heroPrev")?.addEventListener("click",(O)=>{O.preventDefault(),P(-1)}),document.getElementById("heroNext")?.addEventListener("click",(O)=>{O.preventDefault(),P(1)}),q.addEventListener("mouseenter",L),q.addEventListener("mouseleave",V),C()}function C(){let q=document.getElementById("heroSliderWrapper");if(!q)return;let z=Z*100;q.style.transform=`translateX(-${z}%)`,document.querySelectorAll(".hero-slide").forEach((G,O)=>{G.classList.toggle("active",O===Z)})}function V(){if(j)clearInterval(j);j=setInterval(()=>{Z=(Z+1)%_.length,C()},6000)}function L(){if(j)clearInterval(j),j=null}function P(q){if(Z+=q,Z>=_.length)Z=0;else if(Z<0)Z=_.length-1;C(),L(),V()}function k(q=!1){if(q)W=6;let z=document.getElementById("newsFeed");if(!z)return;z.innerHTML="";let K=document.getElementById("hero"),G=K&&K.style.display!=="none",O=_.map((Q)=>Q.title),J=$.filter((Q)=>{if(G&&O.includes(Q.title))return!1;return!0});J.slice(0,W).forEach((Q)=>{z.innerHTML+=`
    ...
    `});let U=document.getElementById("loadMore");if(U)if(W>=J.length)U.innerHTML="Kembali ke Atas ↑",U.onclick=()=>window.scrollTo({top:0,behavior:"smooth"});else U.innerHTML="Muat Lebih Banyak",U.onclick=()=>{W+=6,k(),A()}}function A(){let q=document.getElementById("sidebarRandom");if(!q)return;q.innerHTML="";let z=_.map((J)=>J.title),K=$.slice(0,W).map((J)=>J.title);[...R.filter((J)=>!K.includes(J.title)&&!z.includes(J.title))].sort(()=>0.5-Math.random()).slice(0,5).forEach((J)=>{let X=(J.summary||"").replace(/"/g,"&quot;"),U=J.title.replace(/"/g,"&quot;");q.innerHTML+=`
