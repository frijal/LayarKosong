(()=>{var R=[],Z=[],_=[],W=0,$=null,V=6;function v(q){if(!q)return"Lainnya";return q.split("-").map((z)=>z.charAt(0).toUpperCase()+z.slice(1)).join(" ")}async function M(){try{let q=await window.siteDataProvider.getFor("homepage.ts");R=[];for(let j in q){let G=j.toLowerCase().replace(/\s+/g,"-"),O=v(j);q[j].forEach((K)=>{let U=K.id.replace(/\.html$/,""),Q=K.image||"/thumbnail.webp",J=Q.replace(/\.(jpg|jpeg|png|webp)$/i,"-sm.webp"),L=K.title.replace(/\s*-\s*Layar Kosong$/i,"");R.push({category:O,title:L,id:K.id,url:`/${G}/${U}`,img:J,fullImg:Q,date:K.date?new Date(K.date):new Date,summary:K.description||""})})}R.sort((j,G)=>G.date.getTime()-j.date.getTime()),Z=[...R],_=[...new Set(R.map((j)=>j.category))].map((j)=>R.find((G)=>G.category===j)),f(),A()}catch(q){console.error("Gagal ambil data via provider",q);let z=document.getElementById("newsFeed");if(z)z.innerHTML="<p>Gagal memuat konten.</p>"}}function f(){F(),b(),g(),X(),k();let q=document.getElementById("searchInput"),z=document.getElementById("clearSearch"),j=document.getElementById("hero"),G=document.getElementById("searchForm");if(q)q.addEventListener("input",(U)=>{let Q=U.target.value.toLowerCase();if(Q.length>0){if(j)j.style.display="none";Y()}else{if(j)j.style.display="block";A()}Z=R.filter((J)=>J.title.toLowerCase().includes(Q)||J.summary&&J.summary.toLowerCase().includes(Q)),k(!0),X()});if(G&&q)G.addEventListener("submit",(U)=>{U.preventDefault();let Q=q.value.trim();if(Q.length>0)window.location.href=`/search/?q=${encodeURIComponent(Q)}`});if(z&&q)z.addEventListener("click",()=>{if(q.value="",j)j.style.display="block";Z=[...R],k(!0),X(),A(),q.focus()});let O=document.getElementById("yearFilter"),K=document.getElementById("monthFilter");if(O)O.addEventListener("change",()=>{C(),B()});if(K)K.addEventListener("change",B)}function X(q){let z=document.getElementById("sidebarRandom");if(!z)return;z.innerHTML="";let j=document.querySelector(".pill.active"),G=q||(j?j.textContent?.trim():"All"),O=G==="All"||G==="Kategori"?[...R]:R.filter((J)=>J.category===G),K=Z.slice(0,V).map((J)=>J.title),Q=[...O.filter((J)=>!K.includes(J.title))].sort(()=>0.5-Math.random()).slice(0,10);z.innerHTML=Q.map((J)=>{let L=(J.summary||"").replace(/"/g,"&quot;"),T=J.title.replace(/"/g,"&quot;"),N=J.date,x=`${String(N.getDate()).padStart(2,"0")}.${String(N.getMonth()+1).padStart(2,"0")}.${String(N.getFullYear())}`;return`
