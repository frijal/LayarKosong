...
async function postToLinkedIn() {
  ...
  if (allPosts.length === 0) {
    console.log(
      "🏁 LinkedIn: Semua artikel sudah terposting (berdasarkan cek slug).",
    );
    return;
  }
  ...
  try {
    ...
    const imageResponse = await axios.get(target.image, {
    ...
    console.log(
      `✅ Berhasil! Artikel "${target.title}" sudah tayang di LinkedIn.`,
    );
  } catch (err) {
    console.error(
      "❌ LinkedIn Error:",
