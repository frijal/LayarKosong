...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        document.getElementById('filter-month').addEventListener('change', handleMonthChange);
...
/* ==========================================
   LOGIKA OPTIMASI GAMBAR (PINTAR & 16:9)
   ========================================== */
function getOptimizedImage(src) {
    if (!src) return '/thumbnail.webp';
...
function renderGrid(articles, container) {
    container.innerHTML = '';
    articles.forEach(a => {
        // Optimasi gambar dilakukan di sini sebelum render
        const optimizedSrc = getOptimizedImage(a.image);
