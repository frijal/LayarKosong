(()=>{(function(){function _(V){if(!V)return"Tanpa Judul";return V.replace(/\s*-\s*Layar Kosong$/i,"")}function k(){return window.matchMedia("(max-width: 1024px)").matches}function p(V){return V?V.replace(/\.html$/,""):""}function F(){let Z=window.location.pathname.split("/").filter(Boolean).pop();if(!Z||Z==="artikel")return"";return Z.endsWith(".html")?Z:`${Z}.html`}function B(V){if(!V)return"Lainnya";return V.split("-").map((Z)=>Z.charAt(0).toUpperCase()+Z.slice(1)).join(" ")}function E(V,Z){for(let[J,X]of Object.entries(Z))if(X.some(($)=>$.id===V))return{name:B(J),slug:J,list:X};let Y=window.location.pathname.split("/").filter(Boolean);if(Y.length>0){let J=Y[0];for(let[X,$]of Object.entries(Z))if(X===J)return{name:B(X),slug:X,list:$}}return null}let N="img-broken-placeholder",b=!1;function L(){if(b)return;b=!0;let V=document.createElement("style");V.textContent=`
  .${N} {
...
<a href="https://share.flipboard.com/bookmarklet/popout?v=2&title=${z}&url=${O}&utm_source=dalam.web.id" onclick="window.open(this.href,'_blank','noopener,noreferrer,width=600,height=600');return false;" title="Bagikan ke Flipboard" aria-label="Bagikan ke Flipboard"><svg width="20" height="20" viewBox="0 0 122.88 122.88"><path fill="#F52828" d="M0,0v122.88h122.88V0H0L0,0z M98.3,49.15H73.73v24.58H49.15V98.3H24.58V24.58H98.3V49.15L98.3,49.15z"/></svg></a>
  ...
  `}).join(""),Z.querySelectorAll(".lk-related-thumb[data-fallback-idx]").forEach((Q)=>{let z=Number(Q.dataset.fallbackIdx),O=$[z];if(!O)return;let G=O.image||X;S(Q,[G,X])})}catch(Y){console.error(Y),Z.style.display="none"}}function v(V,Z){function Y(J){if(J==="down"){window.location.href="/";return}if(J==="up"){let $=E(Z,V);if($)window.location.href=`/${$.slug}`;return}let X=document.querySelector(`link[rel="${J}"]`);if(X){let $=X.getAttribute("href");if($)window.location.href=$}}document.addEventListener("keydown",(J)=>{if(k())return;let X=document.activeElement;if(X.tagName==="INPUT"||X.tagName==="TEXTAREA"||X.isContentEditable||X.closest("#disqus_thread"))return;if(J.ctrlKey&&J.key==="ArrowDown")J.preventDefault(),Y("down");if(J.ctrlKey&&J.key==="ArrowUp")J.preventDefault(),Y("up");if(J.ctrlKey&&J.key==="ArrowRight")J.preventDefault(),Y("next");if(J.ctrlKey&&J.key==="ArrowLeft")J.preventDefault(),Y("prev")})}let R=[];function f(){if(document.getElementById("playground-styles"))return;let V=document.createElement("style");V.id="playground-styles",V.innerHTML=`
:root {
  --thumb-size: 4.5rem;
  --item-gap: 1.25rem;
}
...
`,document.head.appendChild(V)}function P(V){V.innerHTML="",[...R].sort(()=>0.5-Math.random()).slice(0,7).forEach((Y)=>{let J=document.createElement("a");J.href=Y.url||"#",J.className="playground-item";let X=Y.image||Y.thumbnail||"",$=X,q=X.lastIndexOf(".");if(q!==-1)$=X.substring(0,q)+"-sm.webp";let Q=_(Y.title);J.innerHTML=`
  <img
  ...
  `,V.appendChild(J)})}async function C(){f();let V=document.getElementById("random-playground-widget");if(!V){V=document.createElement("aside"),V.id="random-playground-widget";let Y=document.createElement("div");Y.id="playground-list";let J=document.createElement("div");J.id="playground-controls";let X=document.createElement("button");X.id="hide-btn",X.textContent="❌",X.title="Sembunyikan widget",X.setAttribute("aria-label","Sembunyikan widget"),X.onclick=()=>{V.style.display="none"};let $=document.createElement("button");if($.id="shuffle-btn",$.textContent="♻️ Acak Artikel",$.onclick=()=>{P(Y)},J.appendChild(X),J.appendChild($),V.appendChild(Y),V.appendChild(J),window.matchMedia("(max-width: 1024px)").matches)document.body.appendChild(V);else{let Q=document.querySelector(".article-content, main, article, #main-wrapper");if(Q&&Q.parentNode)Q.parentNode.insertBefore(V,Q.nextSibling);else document.body.appendChild(V)}}if(typeof R>"u"||R.length===0)try{let Y=await window.siteDataProvider.getFor("pemandu.ts");if(Y){R=[];for(let[J,X]of Object.entries(Y))X.forEach(($)=>{let q=$.id?$.id.replace(".html",""):"",Q=$.url||`/${J}/${q}`;R.push({...$,url:Q})})}}catch(Y){console.error("Gagal memuat data playground:",Y);return}let Z=document.getElementById("playground-list");if(Z&&typeof P==="function")P(Z)}async function w(){try{while(!window.siteDataProvider)await new Promise((Y)=>setTimeout(Y,100));let V=await window.siteDataProvider.getFor("pemandu.ts"),Z=F();if(V&&Object.keys(V).length>0)D(),x(),A(),y(V,Z),v(V,Z);I(Z),C()}catch(V){console.error("Gagal inisialisasi Pemandu:",V)}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",w):w()})();})();
