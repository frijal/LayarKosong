(()=>{(function(){function E(){return window.innerWidth<=768||"ontouchstart"in window||navigator.maxTouchPoints>0}function v(J){return J?J.replace(/\.html$/,""):""}function b(){let X=window.location.pathname.split("/").filter(Boolean).pop();if(!X||X==="artikel")return"";return X.endsWith(".html")?X:`${X}.html`}function P(J){if(!J)return"Lainnya";return J.split("-").map((X)=>X.charAt(0).toUpperCase()+X.slice(1)).join(" ")}function M(J,X){for(let[V,Y]of Object.entries(X))if(Y.some(($)=>$.id===J))return{name:P(V),slug:V,list:Y};let Z=window.location.pathname.split("/").filter(Boolean);if(Z.length>0){let V=Z[0];for(let[Y,$]of Object.entries(X))if(Y===V)return{name:P(Y),slug:Y,list:$}}return null}let k="img-broken-placeholder",N=!1;function F(){if(N)return;N=!0;let J=document.createElement("style");J.textContent=`
  ...
  `,document.head.appendChild(J)}function L(J,X){let Z=[...new Set(X.filter(Boolean))],V=0;function Y(){if(V<Z.length)J.src=Z[V],V++;else J.removeEventListener("error",Y),F(),J.classList.add(k),J.removeAttribute("src"),J.alt=J.alt||"Gambar tidak tersedia"}J.addEventListener("error",Y)}function A(){let J=document.getElementById("progress");if(!J)return;let X=()=>{let{documentElement:Z,body:V}=document,Y=Z.scrollTop||V.scrollTop,$=Z.scrollHeight||V.scrollHeight,q=Z.clientHeight,Q=$-q;J.style.width=Q>0?Y/Q*100+"%":"0%"};window.addEventListener("scroll",X,{passive:!0}),X()}function x(){let J=document.querySelector(".search-floating-container"),X=document.getElementById("floatingSearchInput"),Z=J?.querySelector(".clear-button"),V=J?.querySelector(".floating-results-container");if(!J||!X||!Z||!V)return;if(!Z.innerHTML.trim())Z.innerHTML="❌";let Y,$=(q)=>q.replace(/[&<>'"]/g,(Q)=>({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"})[Q]||Q);X.addEventListener("input",()=>{let q=X.value.trim();if(Z.style.display=q.length?"block":"none",q.length<3){V.style.display="none";return}clearTimeout(Y),Y=setTimeout(async()=>{try{V.innerHTML='<div class="no-results">⏳ Memindai data...</div>',V.style.display="block";let Q=encodeURIComponent(q),z=await fetch(`/cari?q=${Q}&page=1&limit=10`);if(!z.ok)throw Error(`HTTP Error: ${z.status}`);let O=await z.json(),U=O.results||O.data||[];if(U.length>0)V.innerHTML=U.map((G)=>{let R=G.id?G.id.replace(".html",""):"tanpa-judul",H=(G.category||"Lainnya").toLowerCase().replace(/\s+/g,"-"),j=G.snippet_text?G.snippet_text.substring(0,60)+"...":"Lihat artikel selengkapnya";return`<a href="/${H}/${R}"><strong>${$(G.title||"Tanpa Judul")}</strong><small>${$(j)}</small></a>`}).join("");else V.innerHTML=`<div class="no-results">❌ Pencarian "${$(q)}" nihil. Tekan Enter untuk detail.</div>`}catch(Q){console.error("❌ Gagal fetch Floating Search D1:",Q),V.innerHTML='<div class="no-results">⚠️ Ups, database sedang sibuk. Coba sesaat lagi.</div>'}},300)}),X.addEventListener("keydown",(q)=>{if(q.key==="Enter"){q.preventDefault();let Q=X.value.trim();if(Q)window.location.href=`/search/?q=${encodeURIComponent(Q)}`}}),Z.addEventListener("click",()=>{X.value="",V.style.display="none",Z.style.display="none",clearTimeout(Y),X.focus()})}function C(J,X){let Z=document.getElementById("related-articles-grid");if(!Z)return;let V=M(X,J);if(!V)return;let Y=V.list.find((W)=>W.id===X),$=Y?Y.title:document.title,q="";if(Y&&Y.description)q=Y.description;else{let W=document.querySelector('meta[name="description"]');if(W)q=W.getAttribute("content")||""}let Q=q?q:$,z=encodeURIComponent(Q),O=encodeURIComponent(window.location.href),U=document.getElementById("dynamic-nav-container");if(!U)if(U=document.createElement("div"),U.id="dynamic-nav-container",U.className="floating-nav",Z.parentNode)Z.parentNode.insertBefore(U,Z.nextSibling);else document.body.appendChild(U);let G=document.querySelector('link[rel="prev"]'),R=document.querySelector('link[rel="next"]'),_="";if(G){let W=G.getAttribute("title")||"Artikel Sebelumnya";_+=`<a href="${G.getAttribute("href")}" title="${W}" class="btn-emoji">⏪</a>`}if(R){let W=R.getAttribute("title")||"Artikel Selanjutnya";_+=`<a href="${R.getAttribute("href")}" title="${W}" class="btn-emoji">⏩</a>`}U.innerHTML=`
<div class="nav-left">
...
<a href="https://share.flipboard.com/bookmarklet/popout?v=2&title=${z}&url=${O}&utm_source=dalam.web.id" onclick="window.open(this.href,'_blank','noopener,noreferrer,width=600,height=600');return false;" title="Bagikan ke Flipboard" aria-label="Bagikan ke Flipboard"><svg width="20" height="20" viewBox="0 0 122.88 122.88"><path fill="#F52828" d="M0,0v122.88h122.88V0H0L0,0z M98.3,49.15H73.73v24.58H49.15V98.3H24.58V24.58H98.3V49.15L98.3,49.15z"/></svg></a>
...
</div>`;let H=document.getElementById("btn-share-main"),j=document.getElementById("lk-share-providers");if(H&&j)H.addEventListener("click",async()=>{if(navigator.share&&E())try{await navigator.share({title:$,text:Q,url:window.location.href})}catch(W){console.error(W)}else{let W=j.classList.contains("lk-share-providers-hidden");j.classList.toggle("lk-share-providers-hidden",!W),j.classList.toggle("lk-share-providers-visible",W)}})}function y(){let J=document.getElementById("internal-nav");if(!J)return;let Z=Array.from(document.querySelectorAll("h2, h3, h4")).filter((V)=>{return(V.textContent||"").trim().length>0&&!V.closest(".floating-nav")&&!J.contains(V)});if(Z.length===0){J.style.display="none";return}J.innerHTML='<ul class="nav-list">'+Z.map((V,Y)=>{let $=(V.textContent||"").trim();if(!V.id)V.id=$.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/(^-|-$)/g,"")||`section-${Y}`;return`<li class="nav-item nav-${V.tagName.toLowerCase()}"><a href="#${V.id}" class="nav-link">${$}</a></li>`}).join("")+"</ul>",J.addEventListener("click",(V)=>{let Y=V.target;if(Y.tagName.toLowerCase()==="a"&&Y.classList.contains("nav-link")){let $=Y.getAttribute("href");if(!$||!$.startsWith("#"))return;let q=$.substring(1),Q=document.getElementById(q);if(Q){let z=Q.closest("details");while(z){if(!z.open)z.open=!0;z=z.parentElement?.closest("details")||null}}}})}async function w(J){let X=document.getElementById("related-articles-grid");if(!X)return;try{let Z=await window.siteDataProvider.getRelatedLiteData();if(!Z||Object.keys(Z).length===0){X.style.display="none";return}let V=M(J,Z);if(!V){X.style.display="none";return}let Y="/thumbnail-sm.webp",$=V.list.filter((Q)=>Q.id!==J).sort(()=>0.5-Math.random()).slice(0,6);if($.length===0){X.style.display="none";return}X.innerHTML=$.map((Q,z)=>{let O=Q.image?`${Q.image.replace(/\.[^/.]+$/,"")}-rg.webp`:Y;return`
  <div class="rel-card-mini">
  <a href="${`/${V.slug}/${Q.id.replace(".html","")}`}">
  ...
  `}).join(""),X.querySelectorAll(".lk-related-thumb[data-fallback-idx]").forEach((Q)=>{let z=Number(Q.dataset.fallbackIdx),O=$[z];if(!O)return;let U=O.image||Y;L(Q,[U,Y])})}catch(Z){console.error(Z),X.style.display="none"}}function D(J,X){function Z(V){if(V==="down"){window.location.href="/";return}if(V==="up"){let $=M(X,J);if($)window.location.href=`/${$.slug}`;return}let Y=document.querySelector(`link[rel="${V}"]`);if(Y){let $=Y.getAttribute("href");if($)window.location.href=$}}document.addEventListener("keydown",(V)=>{if(E())return;let Y=document.activeElement;if(Y.tagName==="INPUT"||Y.tagName==="TEXTAREA"||Y.isContentEditable||Y.closest("#disqus_thread"))return;if(V.ctrlKey&&V.key==="ArrowDown")V.preventDefault(),Z("down");if(V.ctrlKey&&V.key==="ArrowUp")V.preventDefault(),Z("up");if(V.ctrlKey&&V.key==="ArrowRight")V.preventDefault(),Z("next");if(V.ctrlKey&&V.key==="ArrowLeft")V.preventDefault(),Z("prev")})}let K=[];function I(){if(document.getElementById("playground-styles"))return;let J=document.createElement("style");J.id="playground-styles",J.innerHTML=`
:root {
...
`,document.head.appendChild(J)}function B(J){J.innerHTML="";let X=document.createElement("button");X.id="shuffle-btn",X.textContent="\uD83C\uDFB2 Acak Artikel",X.onclick=()=>{J.classList.add("shake-anim"),setTimeout(()=>{B(J),J.classList.remove("shake-anim")},400)},J.appendChild(X),[...K].sort(()=>0.5-Math.random()).slice(0,4).forEach((V)=>{let Y=document.createElement("a");Y.href=V.url||"#",Y.className="playground-item";let $=V.image||V.thumbnail||"",q=$,Q=$.lastIndexOf(".");if(Q!==-1)q=$.substring(0,Q)+"-sm.webp";Y.innerHTML=`
  <img
  class="playground-thumb"
  src="${q}"
  data-orig="${$}"
  onerror="this.onerror=null; this.src=this.dataset.orig;"
  alt="${V.title||"Thumbnail Artikel"}"
  loading="lazy"
  >
  <h4 class="playground-title">${V.title||"Judul Tanpa Kategori"}</h4>
  `,J.appendChild(Y)})}async function f(){I();let J=document.getElementById("random-playground-widget");if(!J)if(J=document.createElement("aside"),J.id="random-playground-widget",window.matchMedia("(max-width: 1024px)").matches)document.body.appendChild(J);else{let Z=document.querySelector(".article-content, main, article, #main-wrapper");if(Z&&Z.parentNode)Z.parentNode.insertBefore(J,Z.nextSibling);else document.body.appendChild(J)}if(typeof K>"u"||K.length===0)try{let X=await window.siteDataProvider.getFor("pemandu.ts");if(X)K=Object.values(X).flat()}catch(X){console.error("Gagal memuat data playground:",X);return}if(typeof B==="function")B(J)}async function S(){try{while(!window.siteDataProvider)await new Promise((Z)=>setTimeout(Z,100));let J=await window.siteDataProvider.getFor("pemandu.ts"),X=b();if(J&&Object.keys(J).length>0)y(),A(),x(),C(J,X),D(J,X);w(X),f()}catch(J){console.error("Gagal inisialisasi Pemandu:",J)}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",S):S()})();})();
