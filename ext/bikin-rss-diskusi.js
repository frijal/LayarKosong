...
async function run() {
  try {
    ...
    const ghCategories = repoRes.repository.id
      ? repoRes.repository.discussionCategories.nodes
      : [];
    ...
    for (const fileName of RSS_FILES) {
      ...
      const targetCategory = ghCategories.find(
        (c) => c.name.toLowerCase() === rawCategory.toLowerCase(),
      );

      if (!targetCategory) {
        console.log(
          `⚠️ Kategori Discussion "${rawCategory}" tidak ditemukan, skipping...`,
        );
        continue;
      }
      ...
      for (const item of feed.items) {
        ...
        if (checkRes.search.discussionCount > 0) {
          console.log(
            `⏭️ Skip: ${item.title} (Sudah ada diskusi dengan slug ini)`,
          );
          continue;
        }
        ...
        const itemCategory =
          item.categories && item.categories[0]
            ? item.categories[0]
            : rawCategory;
        let existingLabel = ghLabels.find(
          (l) => l.name.toLowerCase() === itemCategory.toLowerCase(),
        );

        if (!existingLabel) {
          try {
            const newLabel = await octokit.request(
