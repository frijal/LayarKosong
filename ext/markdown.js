/*!
 * markdown.js v13.0 — Auto Detect Markdown
 * TANPA class="md" | Aman untuk HTML lama
...
(function () {
  ...
  function processCell(text) {
    ...
    while (i < lines.length) {
      ...
      if (line.trim().startsWith('```')) {
        ...
        i++;
        blocks.push(`<pre><code class="language-${lang}">${escapeHTML(code.join('\n'))}</code></pre>`);
        continue;
      }
      ...
      if (cbMatch) {
        ...
        const label = processInline(escapeHTML(cbMatch[3]));
        const id = genId();
      ...
      if (listMatch) {
        ...
        const item = processInline(escapeHTML(listMatch[2]));
        const level = Math.floor(indent / 2);
        while (stack.length > level) { const c = stack.pop().close(); if (stack.length) stack[stack.length-1].items.push(c); else blocks.push(c); }
        const li = `<li>${item}</li>`;
        if (stack.length === level) stack[stack.length-1].items.push(li);
      ...
      if (line.trim()) blocks.push(`<p>${processInline(escapeHTML(line))}</p>`);
      i++;
    }
    close();
    return blocks.length ? blocks.join('') : processInline(escapeHTML(text));
  }
  ...
  function enhanceNode(node) {
    ...
    // Pastikan parent bukan elemen yang sudah diformat
    ...
    while (i < lines.length) {
      ...
      if (heading) { blocks.push(`<h${heading[1].length}>${processInline(escapeHTML(heading[2]))}</h${heading[1].length}>`); i++; continue; }
      ...
      if (/^\s*[-*+]\s/.test(raw)) {
        ...
        while (i < lines.length && /^\s*[-*+]\s/.test(lines[i])) {
          ...
          if (cb) {
            ...
            const label = processInline(escapeHTML(cb[2]));
            const id = genId();
            const attr = checkboxStates.has(id) ? (checkboxStates.get(id) ? 'checked' : '') : (checked ? 'checked' : '');
      ...
      if (trimmed.startsWith('>')) {
        blocks.push(`<blockquote>${processInline(escapeHTML(trimmed.slice(1)))}</blockquote>`);
        i++;
        continue;
      }

      if (trimmed) blocks.push(`<p>${processInline(escapeHTML(raw))}</p>`);
      i++;
    }
