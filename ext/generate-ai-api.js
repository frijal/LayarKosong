// =========================================================
// SCRIPT DEBUG + HIGHLIGHT: ext/generate-ai-api-logdiff.js
// =========================================================
...
const logMsgs = [];
function log(msg){ console.log(msg); logMsgs.push(msg.replace(/`/g,'')); }
...
for(let i=1;i<=20;i++){const k=process.env[`GEMINI_API_KEY${i}`]; if(k) apiKeys.push(k);}
...
function getAIInstance(key){ return new GoogleGenAI({apiKey:key}); }
...
async function extractContent(file){
  ...
  }catch(e){ log(`⚠️ Gagal baca ${file}: ${e.message}`); return null; }
}
...
async function getWhitelist(){
  ...
  }catch{ log("⚠️ llms.txt tidak ditemukan → semua artikel dilewati"); return new Set(); }
}
...
`;
    try{
      const res = await ai.models.generateContent({model,contents:[{role:"user",parts:[{text:prompt}]}],config:{temperature:0.2}});
...
const tableLines = [];
tableLines.push(`# LogAI - Layar Kosong`);
tableLines.push(`Tanggal: ${DATE_NOW}`);
tableLines.push(`| Artikel | Summary Lama | Summary Baru | Keywords Lama | Keywords Baru | Topics Lama | Topics Baru | Prompt Hint |`);
tableLines.push(`|---------|--------------|--------------|---------------|--------------|------------|------------|-------------|`);
...
for(const [category,items] of Object.entries(rawMeta)){
  for(const [title,file,image,date,summary] of items){
    ...
    const oldData = existsSync(outFile)?JSON.parse(await fs.readFile(outFile,'utf8')):null;
    const content = await extractContent(file);
    if(!content){ failedCount++; continue; }

    const ai = await aiExtract(content,title);

    const newSummary = ai?.summary || summary || '';
    const oldKeywords = oldData?.meta?.keywords||[];
    const oldTopics = oldData?.meta?.topics||[];
    const newKeywords = ai?.keywords||oldKeywords;
    const newTopics = ai?.topics||oldTopics;
    const promptHint = ai?.prompt_hint || oldData?.meta?.prompt_hint || '';

    // Highlight perbedaan keywords/topics
    const highlightArray=(oldArr,newArr)=>{
      const added = newArr.filter(x=>!oldArr.includes(x)).map(x=>`**${x}**`);
      const removed = oldArr.filter(x=>!newArr.includes(x)).map(x=>`~~${x}~~`);
      const kept = newArr.filter(x=>oldArr.includes(x));
      return [...kept,...added,...removed].join(', ');
    };

    const kHighlighted = highlightArray(oldKeywords,newKeywords);
    const tHighlighted = highlightArray(oldTopics,newTopics);

    const post = {id,slug:id,title,category,published_at:date,url:`${BASE_URL}/artikel/${file}`,image,content,meta:{summary:newSummary,keywords:newKeywords,topics:newTopics,prompt_hint:promptHint}};
    await fs.writeFile(outFile,JSON.stringify(post,null,2));

    tableLines.push(`| ${title} | ${oldData?.meta?.summary||''} | ${newSummary} | ${oldKeywords.join(', ')} | ${kHighlighted} | ${oldTopics.join(', ')} | ${tHighlighted} | ${promptHint} |`);
...
await fs.writeFile(LOG_FILE,tableLines.join('\n'));
