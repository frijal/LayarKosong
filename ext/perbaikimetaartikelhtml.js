...
const REPORT_FILE = `mini/AuditInjectHTML-${new Date().toISOString().slice(0, 10).replace(/-/g, "")}.md`;
...
const strictPostDeduplicator = (h) =>
  dedupeMetaByKey(
    dedupeMetaByKey(dedupeMetaByKey(h, "property", "og:"), "name", "twitter:"),
    "property",
    "article:",
  );
...
function qualityScore(html) {
  ...
  return {
    score,
    max: rules.length,
    percent: Math.round((score / rules.length) * 100),
  };
}
...
function isHtmlAlreadyCompliant(html, json) {
  ...
  if (!html.includes(`rel="canonical"`) || !html.includes(json.url))
    return false;
  if (!same(extractProp(html, "og:image"), json.image)) return false;
  if (
    !same(extractMeta(html, "news_keywords"), normalizeToString(meta.keywords))
  )
    return false;
  ...
  if (
    !new RegExp(`"headline"\\s*:\\s*"${json.title.replace(/"/g, '\\"')}"`).test(
      html,
    )
  )
    return false;
