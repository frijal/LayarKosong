  ...
  try {
    ...
    for (const f of files) {
      ...
      if (!/<meta[^>]+property=["']og:image:alt["']/i.test(html))
        metaTags.push(`<meta property="og:image:alt" content="${title}">`);
      ...
      if (!/<meta[^>]+name=["']twitter:image["']/i.test(html))
        metaTags.push(`<meta name="twitter:image" content="${finalImage}">`);

      if (!/<meta[^>]+itemprop=["']image["']/i.test(html))
        metaTags.push(`<meta itemprop="image" content="${finalImage}">`);
