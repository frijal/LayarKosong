(()=>{var $=[];async function L(){try{let K=await window.siteDataProvider.getFor("homepage.ts"),z={};$=[];for(let P in K){let Q=P.toLowerCase().replace(/\s+/g,"-"),q=N(P);z[q]=[],K[P].forEach((J)=>{let M=J.id.replace(/\.html$/,""),V=(J.image||"/thumbnail.webp").replace(/\.(jpg|jpeg|png|webp)$/i,"-rg.webp"),W=J.title.replace(/\s*-\s*Layar Kosong$/i,""),_=J.description||"",Z={category:q,categorySlug:Q,title:W,url:`/${Q}/${M}`,img:V,date:J.date?new Date(J.date):new Date,summary:_};z[q].push(Z),$.push(Z)}),z[q].sort((J,M)=>M.date.getTime()-J.date.getTime())}x(z),y()}catch(K){console.error("Gagal menyajikan menu utama, Chef:",K);let z=document.getElementById("railContainer");if(z)z.innerHTML="<p class='intro'>Data sedang mogok kerja. Coba muat ulang halaman.</p>"}}function N(K){if(!K)return"Lainnya";return K.split("-").map((z)=>z.charAt(0).toUpperCase()+z.slice(1)).join(" ")}function j(K){let z=document.getElementById("imageLightbox"),P=document.getElementById("lightboxImg");if(z&&P)P.src=K,z.showModal(),z.addEventListener("click",(Q)=>{let q=z.getBoundingClientRect();if(!(q.top<=Q.clientY&&Q.clientY<=q.top+q.height&&q.left<=Q.clientX&&Q.clientX<=q.left+q.width))z.close()})}window.openLightbox=j;function x(K){let z=document.getElementById("railContainer");if(!z)return;let P="",Q=Object.keys(K).sort((q,J)=>{let M=K[q][0]?.date.getTime()||0;return(K[J][0]?.date.getTime()||0)-M});for(let q=0;q<Q.length;q++){let J=Q[q],M=K[J].slice(0,4);if(M.length===0)continue;let Y=M[0].categorySlug;if(P+=`
