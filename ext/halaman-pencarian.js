document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!query) {
    ...
    resultsContainer.innerHTML =
      '<p class="text-center text-red-500 py-10">Silakan ketik kata kunci pada kolom pencarian...</p>';
    return;
  }
  ...
  function renderResults(matches, query) {
    ...
    matches.forEach((m) => {
      ...
      const date = m.dateISO
        ? new Date(m.dateISO).toLocaleDateString("id-ID", {
            day: "numeric",
            month: "short",
            year: "numeric",
          })
      ...
      card.style.cssText =
        "display:flex; flex-direction:column; border-radius:8px; overflow:hidden; height:100%;";
